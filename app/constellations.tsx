...
export default function ConstellationsScreen() {
  ...
  // Suggested groupings derived from patterns in your memories (shared place,
  // shared people, same month). Dismissable.
  ...
  const suggestions = useMemo(() => {
    const raw = suggestConstellations();
    return raw
      .map((s) => ({
        id: s.id,
        name: s.reason,
      ...
      .filter(
        (s) =>
          !dismissed.includes(s.id) && !existingSignatures.has(s.starIds.slice().sort().join('|')),
      );
  }, [suggestConstellations, dismissed, existingSignatures]);
            ...
            {suggestions.length > 0 && (
              <View className="mb-6 gap-2.5">
                <View className="flex-row items-center gap-2">
                  ...
                  <Text className="text-starlight flex-1 font-semibold">Suggested for you</Text>
                </View>
                <Text className="text-muted text-xs leading-4">
                  Memoria spots patterns in your memories and proposes constellations.
                </Text>
